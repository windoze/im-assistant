...
- **完成记录(2026-07-07)**:
  - 已检查当前 capability/tool 实现与调用边界,现有系统工具均为钉钉 OpenAPI/通讯录/文档/待办/通知/日历总结等固定 handler,未引入执行任意代码或脚本的工具。
  - 当前无需创建子进程/容器沙箱;README 已记录该状态,并明确后续若新增任意代码/脚本执行工具,必须先实现受限 FS、独立 workdir、上下文与凭证视图隔离后才能暴露给会话。
  - 本任务仅更新文档与 TODO 记录,未改动运行时代码;复用 T37 已完成的全量绿色验证结果,未重新运行完整测试套件。

## [DONE] T39 【REVIEW】M7 加固 + 全系统终审
...
- **完成记录(2026-07-07)**:
  - 已终审 T35–T38 的审计日志、OBO 授权审计、confirm/consent 决议审计、指令审计、Stream 重连、`msg_id` 幂等、Session/pending 重启恢复、高敏感能力强制确认、运行时指标和沙箱状态;与 `docs/architecture.md` §8/§9 对齐,未发现重大架构偏离。
  - 问题清单:发现进程在入站 `msg_id` claim 后、标记 processed 前退出时,SQLite 中残留的 `processing` 幂等记录会让重启后的钉钉重试被误判为重复消息并跳过。已新增启动恢复逻辑,在 Stream 启动时释放钉钉平台遗留的 `processing` 入站 claim,保留已 `processed` 记录继续去重,并补充 store/startup 回归测试和 README 说明。
  - 已回归覆盖 M1 对话入口、M3 capability/tool 执行与建文档工具路径、M4 日程总结 OBO/授权/静默刷新路径、M5 confirm/取消/超时、M6 指令通道、M7 审计/重连/幂等/恢复/高敏边界;外部真实钉钉端到端仍受当前环境缺少 `.env` 凭据、OAuth 公网回调、文档父级配置和日历权限限制,未对真实账号执行发送/建文档/授权读日程人工验证。
  - 遗留问题清单:未发现需要新增 TODO 的未排期代码缺陷;若未来新增任意代码/脚本执行工具,仍须先按 T38 记录实现受限 FS、独立 workdir、上下文与凭证视图隔离。
  - 已验证:`.venv/bin/ruff format .`、`.venv/bin/ruff check .`、`.venv/bin/pytest -q`。
