...
canUse(cap, mode, actor, channel):
    ...
    if mode == DM    and dm in cap.availableIn:
        if cap.origin == user:                          return actor 拥有该私有能力
        return cap.origin in {system, base}
    if mode == Group and group in cap.availableIn
       and cap in channel.enabledCaps:                  return true
    return false
```
